...
export class aButton {
    ...
    setData = function(pressed:number):void {
        this.data = pressed
    }

    getPressed = function():number[] {
        return [this.data]
    }
...
export class accelerometerData {
    ...
    getScaledData(): number[]{
        let scaledData = [this.getScaledX(), this.getScaledY(), this.getScaledZ(), this.getScaledS()] 
        return scaledData.reduce(
            (a, b) => {
                return a.concat(b)
            }
        )
        /*
        ():number[] => [this.getScaledX(), this.getScaledY(), this.getScaledZ(), this.getScaledS()].reduce( 
        (a, b) => {
            return a.concat(b);
        }, [])
        */
